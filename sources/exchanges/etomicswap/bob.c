...
int main(int argc, char** argv)
{
    ...
    switch (action)
    {
        ...
        case BOB_ETH_BALANCE:
            err = 0;
            printf("%" PRIu64 "\n", getEthBalance(bobAddress,&err));
            break;
        case BOB_ERC20_BALANCE:
            err = 0;
            printf("%" PRIu64 "\n", getErc20BalanceSatoshi(bobAddress, tokenAddress,2,&err));
            break;
        case TX_RECEIPT:
            printf("getTxReceipt\n");
    ...
    uint64_t tokenAllowance = getErc20Allowance(bobAddress, bobContractAddress, tokenAddress,2u);
    printf("allowance: %" PRIu64 "\n", tokenAllowance);

    err = 0;
    char *sendEthTx = sendEth(bobAddress, "100000000000000", getenv("BOB_PK"), 0,0,0,err);
    printf("sent ETH: %s\n", sendEthTx);
    free(sendEthTx);

    err = 0;
    char *sendErc20Tx = sendErc20(tokenAddress, bobAddress, "100000000000000", getenv("BOB_PK"), 0,0,0,err,2u);
    printf("sent Erc20: %s\n", sendErc20Tx);
    free(sendErc20Tx);

    uint64_t gasPrice = getGasPriceFromStation(err);

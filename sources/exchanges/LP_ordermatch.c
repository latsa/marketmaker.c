...
char *LP_connectedalice(struct LP_quoteinfo *qp,char *pairstr) // alice
{
    ...
    if ( bits256_nonz(qp->privkey) != 0 )//&& qp->quotetime >= qp->timestamp-3 )
    {
        ...
        else if ( nn_connect(pairsock,pairstr) >= 0 )
        {
            ...
            printf("alice pairstr.(%s) pairsock.%d pthread_t %zd\n",pairstr,pairsock,sizeof(pthread_t));
...
{
    uint64_t desttxfee,txfee/*,balance*/; 
    uint32_t lastnonce; int64_t bestsatoshis=0,destsatoshis; struct iguana_info *basecoin,*relcoin; struct LP_utxoinfo *autxo,B,A; struct LP_quoteinfo Q; bits256 pubkeys[100]; struct LP_address_utxo *utxos[4096]; int32_t num=0,maxiters=100,i,max=(int32_t)(sizeof(utxos)/sizeof(*utxos)); char _uuidstr[65];

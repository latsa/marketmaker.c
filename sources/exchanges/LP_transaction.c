...
{
    static void *ctx;
    cJSON *txobj,*item,*vins; 
    uint8_t addrtype,rmd160[20],pubkey33[33],tmptype,/*data[8192+64],*/script[8192],spendscript[256]; 
    char *coinaddr,*rawtxbytes,*scriptstr,spendscriptstr[128],blastaddr[64],wifstr[64]; 
    bits256 txid; 
    uint32_t locktime,/*crc32,*/timestamp; 
    int64_t change=0,adjust=0,/*total,*/value,amount = 0; 
    int32_t i,/*offset,len,*/ scriptlen,spendlen,suppress_pubkeys,ignore_cltverr,numvouts=0;
...
{
    static void *ctx;
    int32_t broadcast,i,/*num,*/numblast,utxovout,completed=0,numvouts,changeout; 
    char *passphrase,changeaddr[64],vinaddr[64],wifstr[65],blastaddr[65],str[65],*signret,*signedtx=0,*rawtx=0; 
    struct vin_info V; 
    uint32_t /*locktime,*/starttime;
    uint8_t pubkey33[33]; 
    cJSON *retjson,/**item,*/*outputs,*vins=0,*txobj=0,*privkeys=0; 
    struct iguana_msgtx msgtx; 
    bits256 privkey,pubkey,checktxid,utxotxid,signedtxid; 
...
{
    static void *ctx;
    int32_t broadcast,allocated_outputs=0,iter,i,num,utxovout,autofee,completed=0,maxV,numvins,numvouts,datalen,suppress_pubkeys; 
    bits256 privkey; 
    //struct LP_address *ap; 
    char changeaddr[64],vinaddr[64],str[65],wifstr[64],*signret,*signedtx=0,*rawtx=0; struct vin_info *V; uint32_t locktime; cJSON *retjson,*item,*outputs,*vins=0,*txobj=0,*privkeys=0; struct iguana_msgtx msgtx; bits256 utxotxid,signedtxid; uint64_t txfee=0,newtxfee=10000;
...
{
    char *retstr; cJSON *argjson,/**withdrawjson,*/*outputs,*item; 
    int64_t /*total,*/balance,txfee;
    if ( coin->etomic[0] == 0 )
    {
        if ( coin->electrum != 0 )
...
{
    static bits256 zero; 
    bits256 utxotxid,txid; 
    struct iguana_info *coin; 
    cJSON *retjson,*outputs,*argjson,*txids,*unspents,*item,/**gen,*/*output; 
    int32_t i,n,utxovout; 
    char *retstr,*hexstr;

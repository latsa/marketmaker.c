...
// iguana_OS has functions that invoke system calls. Whenever possible stdio
// and similar(?) portable functions are used. OS specific calls are routed to
// iguana_OS_portable_* .
//
// Usually(?), all but one (?) OS can be handled with the same code,
// so iguana_OS_portable.c has most of this shared logic and an
// #ifdef iguana_OS_nonportable.c
//
// iguana_OS_portable.c contains the shared logic. OS specific functionality
// is implemented in iguana_OS_nonportable.c

// time functions are treated separately

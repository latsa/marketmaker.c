...
SOCKET iguana_socket(int32_t bindflag,char *hostname,uint16_t port)
{
   SOCKET sock;
    int32_t opt,result; 
    ...
    else
    {
        while ( (result= bind(sock,(struct sockaddr*)&saddr,addrlen)) != 0 )
        {
            if ( errno == EADDRINUSE )
            {
                ...
                sleep(3);
                printf("%s(%s) port.%d try again: %s sock.%lld. errno.%d\n",bindflag!=0?"bind":"connect",hostname,port,strerror(errno),sock,errno);
                if ( bindflag == 1 )
                {
                    closesocket(sock);
                    return(-1);
        ...
        {
            printf("listen(%s) port.%d failed: %s sock.%lld. errno.%d\n",hostname,port,strerror(errno),sock,errno);
            if ( sock >= 0 )
                closesocket(sock);
            return(-1);
        }
...
char *stats_rpcparse(char *retbuf,int32_t bufsize,int32_t *jsonflagp,int32_t *postflagp,char *urlstr,char *remoteaddr,char *filetype,uint16_t port)
{
    ...
    long filesize; 
    char symbol[64],buf[4096],*userpass=0,urlmethod[16],*data,url[8192],furl[8192],*retstr=0,*filestr,*token = 0; 
    int32_t i,j,n,num=0; 
    uint32_t queueid;
...
size_t iguana_getcontentlen(char *buf,size_t recvlen)
{
...
size_t iguana_getheadersize(char *buf,size_t recvlen) {
   char *delim = "\r\n\r\n";
   char *str = strstr(buf, delim);

   return str ? str - buf + strlen(delim) : recvlen;
}
...
void LP_rpc_processreq(void *_ptr)
{
    ...
    SOCKET sock;
    int32_t flag,postflag=0,numsent,jsonflag=0;
    size_t hdrsize, recvlen =0, contentlen, remains, len;
    ...
    while ( remains > 0 )
    {
        //printf("flag.%d remains.%zd recvlen.%zd\n",flag,remains,recvlen);
        if ( (len= (size_t)recv(sock,buf,(int)remains,0)) < 0 )
        {
            if ( errno == EAGAIN )
            {
                printf("EAGAIN for len %zd, remains.%zd\n",len,remains);
                usleep(10000);
            }
        ...
        else
        {
            ...
            {
                usleep(10000);
                printf("got.(%s) %zd remains.%zd of total.%zd\n",jsonbuf,recvlen,remains,len);
                if ( flag == 0 )
                    break;
            }
        }
    ...
    if ( retstr != 0 )
    {
        ...
        while ( remains > 0 )
        {
            if ( (numsent= (int32_t)send(sock,&retstr[i],(int)remains,MSG_NOSIGNAL)) < 0 )
            {
            ...
            else if ( remains > 0 )
            {
                ...
                i += numsent;
                if ( remains > 0 )
                    printf("iguana sent.%d remains.%zd of recvlen.%zd (%s)\n",numsent,remains,recvlen,jsonbuf);
            }
        }
...
void stats_rpcloop(void *args) {
    ...
    while ( LP_STOP_RECEIVED == 0 )//LP_bindsock_reset == initial_bindsock_reset )
    {
...
#endif
            //if ( counter++ < 1 )
                printf(">>>>>>>>>> DEX stats 127.0.0.1:%d bind sock.%lld DEX stats API enabled at unixtime.%u <<<<<<<<<\n",port,bindsock,(uint32_t)time(NULL));
        }
        //printf("after sock.%d\n",sock);
        ...
        if ( sock < 0 )
        {
            printf("iguana_rpcloop ERROR on accept port.%u usock.%lld errno %d %s\n",port,sock,errno,strerror(errno));

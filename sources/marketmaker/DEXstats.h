...
float _calc_pricey(register double price,register double weekave)
{
    if ( price != 0. && weekave != 0. )
        return (float) (0.1 * calc_loganswer(weekave,price));
    else 
       return(0.f);
}
...
{
    int32_t i,n; 
    struct DEXstats_priceinfo *pp; uint32_t *utc32,tmp,timestamp,lefttimestamp,righttimestamp; double *splinevals,total; char fname[1024]; cJSON *retjson,*array,*item;
    ...
    for (i=0; i<Num_priceinfos; i++)
        ...
        {
            pp = &Prices[i];
            for (int32_t j=0; j<=pp->numdates; j++)
            {
    ...
    if ( n > 3 )
    {
        ...
        tmpptr = bytemap;
        for (uint32_t j=0; j<height*numdates*24; j++)
        {
            val = bitmap[j];
            red = val & 0xff;
            green = (val >> 8) & 0xff;
